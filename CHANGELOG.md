...
- **Le journal des frictions rejoint le site**, comme septième page dérivée de `FRICTIONS.md` par le même script que les autres. Il vivait sur GitHub, cité par la documentation mais jamais publié avec elle : une évaluation externe faite depuis le site et npm a conclu à un outillage immature sans avoir pu voir les vingt et une frictions ni les dix-neuf lots d'usage qui les ont produites. La preuve existait, elle n'était pas là où on regarde.
- **La version du standard apparaît dans le pied de page**, lue dans le `CHANGELOG` plutôt que recopiée : un numéro écrit à la main dans un composant se périme au premier oubli, et rien ne casse, la page affiche simplement un chiffre faux. Les entrées non publiées sont ignorées, le lecteur voulant savoir quelle version il lit.
- **Le registre dit que son tableau n'est pas la liste complète.** Les six Règles de conception ne font échouer aucun lint, et c'est ce qui les rend faciles à ignorer : un projet réel a passé dix-neuf lots sans appliquer la Règle de Logique Extraite pendant que les neuf codes restaient au vert. L'encadré suit le tableau, là où le lecteur s'arrête.
- **La page d'adoption dit ce que la migration coûte**, et surtout ce qu'on ne peut pas chiffrer à la place de l'équipe. Le tableau sépare ce que `--fix` corrige de ce qui demande une décision : les trois règles de frontière sont les seules qui ne se corrigent pas en série, une violation disant qu'un fichier est au mauvais endroit sans dire où il doit aller. La section reconnaît qu'aucun retour public de migration chiffrée n'existe à ce jour.
- **Le corpus tranche la contradiction entre Lazy Abstraction et testabilité.** La règle autorise l'accès direct au client de base de données tant qu'il n'y a qu'un fournisseur ; le standard promet par ailleurs que `core/` se teste sans mock. Prises à la lettre, les deux se contredisent, et un service qui importe `db` directement satisfait la première en cassant la seconde. Le corpus dit désormais que le service reçoit son dépôt en paramètre, décrit par un **type structurel écrit sur place** : trois méthodes à côté de la fonction qui les emploie, aucun fichier créé, aucune inversion de dépendance à maintenir. Ce n'est pas un `contract.ts` déguisé, et la section explique pourquoi, avec le tableau de ce que chaque approche coûte. Remonté par une évaluation externe du standard.
- **`fromThrowable` complète le Result Pattern à sa frontière.** Les cinq helpers supposaient tous manipuler des `Result`, mais rien ne disait comment convertir ce que le monde extérieur lève : une base de données jette sur une contrainte violée, un `fetch` sur un délai dépassé. Sans convention, chaque dépôt improvisait son `try/catch`, et il suffisait d'un oubli pour qu'une exception traverse un `andThen` qui croyait n'enchaîner que des résultats. Le helper est livré avec quatre tests, et le corpus dit son unique endroit légitime : `repository.ts`, jamais plus haut. Sa fonction de conversion est obligatoire, pour forcer à décider ce que l'erreur signifie pour le métier plutôt que de laisser remonter un objet de bibliothèque jusqu'aux écrans.
- **Le corpus dit où vit un cache client.** `useQuery` et `useMutation` vivent dans le `hooks/` de la feature, leur `queryFn` appelle `core/<domaine>/service.ts`, et le hook ne porte aucune règle métier. Un `queryFn` qui contiendrait un calcul remonterait la logique dans `features/`, ce que MA-001 interdit et que le linter ne verrait pas, faute d'import fautif. `create-maedow-arch-app` passe en 0.11.0.
- **Le registre recense les Règles de conception, à part des neuf codes.** Le corpus portait cinq exigences nommées, la Règle d'Or, Zéro Modèle dans le JSX, le Pragmatisme Typé, la Lazy Abstraction et la Dégradation de `features/_shared/`, **dispersées dans trois documents sans qu'aucun index ne les relie ni ne dise ce qui les sépare du registre**. Elles y figurent désormais avec leur exigence et leur emplacement, et le registre explique la frontière : un code se constate, une Règle de conception s'apprécie. C'est pourquoi elles n'entrent pas dans la clause de conformité, qui repose sur des questions à réponse binaire, et pourquoi leur donner un code diluerait le rapport de sept règles vérifiées sur neuf sans rien vérifier de plus.
- **La doctrine des hooks devient la Règle de Logique Extraite.** Elle était énoncée mais innommable : on ne pouvait la citer en revue qu'en renvoyant à une section, et un titre de section se réécrit. Elle rejoint les Règles de conception avec son énoncé en une phrase, et le corpus rappelle que son seuil est indicatif : un écran à quatre états ne viole rien, il mérite un regard.
- **Le scaffold ne génère plus avec des majeures dépassées.** ESLint passe en 10, ce que le lot précédent venait de rendre possible, Next en 16, `@types/node` en 26 et `vitest` en 5. Un projet généré démarrait jusqu'ici avec Next 15 alors que la 16 est publiée depuis un moment. **TypeScript reste en 5 délibérément** : `typescript-eslint` déclare `typescript: >=4.8.4 <6.1.0`, et passer en 7 casserait le lint, c'est-à-dire les sept règles vérifiées par la machine. Voir [F-021](./FRICTIONS.md). `create-maedow-arch-app` passe en 0.10.0.
- **Le premier build d'un projet Next ne réécrit plus son `tsconfig.json`.** Next 16 impose deux valeurs et les applique en cours d'exécution, ce qui faisait échouer le build pendant lequel la réécriture a lieu, et lui seul : sur un poste le défaut disparaît avant qu'on ait fini de lire le message, en intégration continue il est systématique. Le template livre désormais ces valeurs d'avance, `jsx` en `react-jsx` et `.next/dev/types` dans `include`. Vérifié par empreinte sur un projet neuf, le fichier reste intact et aucun message de reconfiguration n'apparaît. Remonté par le projet ABBA, entrée N-002 de son relevé outillage.
- **L'entrée stricte ne verrouille plus les projets sur ESLint 9.** Elle portait MA-007 par `eslint-plugin-import`, plafonné à ESLint 9 et sans aucune préversion pour la 10 sur 132 versions publiées : tout projet appliquant `strict` était bloqué sur une version que son propre éditeur classe en maintenance, et `strict` n'est pas optionnelle puisqu'elle porte trois des neuf règles. Elle emploie désormais `eslint-plugin-import-x`, le fork maintenu, dont les pairs acceptent ESLint 8, 9 et 10. Le nom de la règle change avec le plugin, `import-x/no-cycle`, et le registre le dit. **`eslint-config-maedow-arch` passe en 0.4.0 : un projet qui charge l'entrée stricte doit installer `eslint-plugin-import-x` à la place de `eslint-plugin-import`.** Les commandes du README et de la page d'adoption sont à jour, et le fragment du scaffold l'installe désormais, ce qui fait passer `create-maedow-arch-app` en 0.9.1, ce qui règle du même coup l'ambiguïté du champ `peerDependenciesMeta`, qui déclarait optionnel un paquet que `strict.js` importe. Voir [F-020](./FRICTIONS.md).
- **Une base de composants ne dépose plus ses hooks hors des couches.** L'assistant de shadcn écrit un `components.json` déclarant `"hooks": "@/hooks"`, or `@/hooks` n'est aucune des cinq couches : les fichiers qu'il y dépose échappent à l'audit et à `eslint-plugin-boundaries` à la fois, et rien n'empêche un import d'y remonter le flux. Le corpus dit désormais quel alias poser avant la première installation, et où déplacer un hook déjà déposé. Chez ABBA, le standard n'avait été rétabli que par accident, à la faveur d'une panne sans rapport.
- **`contract.ts` est marqué conditionnel dans l'arborescence type.** La Règle de Lazy Abstraction, énoncée quelques paragraphes plus haut, dit qu'il ne s'introduit qu'à la deuxième implémentation réelle. Un lecteur pressé, humain ou agent, reproduisait l'arborescence telle quelle et créait par défaut ce que la règle proscrit.
- **Le corpus ne se contredit plus sur `result.ts`.** La phrase « la couche naît de son premier habitant » visait le profil Light, mais rien dans sa formulation ne le restreignait, et un lead tech l'a lue comme une consigne de ne pas créer `core/common/result.ts` : deux fichiers livrés par le générateur officiel ont été retirés, et `npm test` est passé au rouge faute de test à exécuter. Le corpus précise que la Lazy Abstraction porte sur les contrats et les adaptateurs, pas sur le type de retour que toute la couche emploie dès sa première fonction.
- **La page d'adoption distingue les deux usages de l'audit.** `npx` pour découvrir ce que l'adoption coûterait, une dépendance de développement pour suivre une migration : `npx` peut réutiliser une version mise en cache, et deux postes rendent alors deux rapports différents sur le même code, ce qui est exactement ce qu'il ne faut pas quand le compte sert à mesurer une progression.
...
- **Deux liens « modifier sur GitHub » rendaient 404.** Le chemin de la source était déduit en remplaçant `.mdx` par `.md`, ce qui vaut pour les quatre pages dérivées du corpus et se trompe pour les autres : `FRICTIONS.md` porte des majuscules, et la page d'adoption n'a pas de source à la racine puisqu'elle est écrite directement dans le dossier de contenu. Celui de la page d'adoption était mort depuis sa création. Une table dit désormais où vit chaque source, et le build échoue si l'une manque : un lien mort ne cassait rien, la page se construisait, le bouton s'affichait, et le 404 n'apparaissait qu'au visiteur qui cliquait.
- **L'audit prenait les génériques TypeScript pour du JSX.** `Promise<UserEntity>`, `Array<string>` et jusqu'à une balise écrite dans un littéral de chaîne étaient signalés « contient du JSX », sur la couche même où vivent les dépôts et les validateurs génériques : l'audit officiel signalait la signature que le corpus donne en exemple. Le projet ABBA a compté quatre, puis huit, puis onze violations MA-004 annoncées, **toutes fausses**, jusqu'à devoir inscrire une décision de projet disant que le critère de vérité est `npx eslint` et non notre compteur. La détection ne s'applique plus qu'aux `.tsx` et `.jsx`, ce que le registre justifiait déjà : un `.ts` ne peut pas contenir de JSX, le parser le refuserait avant nous. Voir [F-019](./FRICTIONS.md). `maedow-arch` passe en 0.1.3.
- **Le rapport signale le code rangé sous `src/` hors de toute couche.** `shadcn init` déclare `"hooks": "@/hooks"` et y dépose les hooks de ses composants : ces fichiers échappent à l'audit et aux frontières ESLint à la fois, aucune politique ne s'y applique, et rien n'empêche un import d'y remonter le flux. Le rapport les nomme désormais, **sans les compter comme des violations** : aucune règle du registre ne porte sur ce cas, et un outil n'invente pas de règle.
- **Les deux commandes de la page d'accueil débordaient du cadre sur un téléphone.** Sans `grid-cols-1`, la colonne implicite se dimensionnait sur le contenu le plus large : la commande de création, qui ne se coupe pas, étirait la grille à 438 pixels dans une fenêtre de 390, et `overflow-hidden` rognait le dépassement en silence. Les deux blocs perdaient leur bouton copier et les phrases leur fin. La piste déclarée vaut `minmax(0, 1fr)`, donc la commande retrouve le défilement horizontal prévu pour elle.
- **La barre de défilement du sommaire occupait la largeur d'un mot, juste à droite du texte.** Dans une colonne étroite, elle se plaçait là où l'œil revient à chaque ligne, et attirait plus l'attention que les entrées qu'elle accompagnait. Elle est masquée : le sommaire porte déjà son propre repère de position, le trait vertical qui marque l'entrée courante. L'utilitaire est réservé aux conteneurs qui ont ce repère, masquer la barre ailleurs revenant à cacher qu'il reste du contenu à lire.
...
- **La composition prescrite éteignait MA-004.** Charger l'entrée stricte après l'entrée par défaut, ce que le corpus recommande, faisait disparaître l'interdiction du JSX dans `core/`. En configuration plate, deux objets qui ciblent le même fichier et déclarent la même règle ne fusionnent pas : le dernier remplace le premier, et l'entrée stricte redéclarait `no-restricted-syntax` sur tous les fichiers TypeScript. Un dernier bloc y redéclare désormais les trois sélecteurs ensemble, et le banc éprouve la fixture invalide sous la composition en plus de l'entrée seule, ce qu'il ne faisait pour aucune des deux. Voir [F-018](./FRICTIONS.md). `eslint-config-maedow-arch` passe en 0.3.1.
- **Un projet généré recevait Tailwind sans jamais l'appliquer.** Le template `framework-next` n'importait pas `globals.css`, là où `framework-vite` et `demo-app-next` le faisaient. La combinaison Next et template blank produisait donc un projet où la feuille était présente, la configuration en place, et aucun style appliqué. Rien ne le signalait : lint, typecheck, build et audit passaient au vert, et le défaut ne se découvrait qu'au premier composant stylé.
...
- **Le site du standard applique enfin le typage strict que le standard exige.** `npx maedow-arch check` le disait sur le dépôt lui-même : `noUncheckedIndexedAccess` et `exactOptionalPropertyTypes` manquaient au `tsconfig.json` du site, alors que les deux modèles de projet générés les activent, en Next comme en Vite. Le site qui publie le standard était le seul à ne pas s'y tenir. La correction tient en un fichier : le sélecteur de profil déclarait ses deux profils dans un tableau, dont le premier sert de repli, et un tableau ne garantit pas son premier élément. Il est désormais typé comme un tuple non vide, ce qui fait dire au type ce que la donnée garantissait déjà, plutôt que de l'affirmer après coup par une assertion.
- **L'audit examine une liste blanche, et non plus tout ce qu'une liste d'exclusions avait oublié d'exclure.** Seuls les fichiers appartenant à une couche du standard sont examinés ; les autres sont dénombrés et rapportés à part, jamais comptés comme violations. Une liste d'exclusions laisse passer par défaut : elle a signalé un `public/mockServiceWorker.js` généré, et aurait signalé demain `scripts/`, `e2e/` ou le prochain dossier que personne n'avait prévu. C'est exactement ce que le corpus interdit à propos du DTO de sortie, un `Omit` étant en retard d'un champ en permanence : l'outil faisait ce que sa propre règle défend. La liste de dossiers ignorés subsiste pour ne pas descendre dans `node_modules`, ce qui est une question de coût et non de périmètre. Le rapport annonce désormais combien de fichiers vivent hors des couches, en texte comme en JSON : sur un projet en migration, c'est un renseignement utile, et il ne se confond plus avec une dette à corriger. `maedow-arch` passe en 0.1.1.
...
### `npx maedow-arch check`, l'audit d'un projet existant

Le troisième paquet du standard, et le premier qui s'adresse aux projets déjà écrits. Il paraît en 0.1.0.

- **L'outillage ne s'adressait qu'aux projets à créer**, alors qu'aucune équipe ne repart de zéro pour adopter une architecture. La commande lit un projet quelconque sans rien installer ni modifier, et sans avoir besoin qu'il compile : un audit qu'il faudrait préparer ne serait jamais lancé par l'équipe qui hésite encore, c'est-à-dire par la seule à qui il s'adresse.
- **Le rapport est ordonné par ce qui débloque le reste**, jamais par gravité, et chaque section dit ce que la correction rend possible. Une équipe qui découvre deux cents violations a besoin de voir un chemin, pas un verdict : le total n'apparaît qu'à la fin.
- **Quatre sorties pour quatre usages.** Lisible par défaut, `--json` pour tracer une courbe en intégration continue, `--seuil` pour mesurer une progression sans rougir un pipeline au premier jour, et `--fix` limité aux options du `tsconfig.json`. Les déplacements de fichiers sont proposés, jamais appliqués : un outil qui déplace du code sans qu'on le lui demande ne se fait pardonner qu'une fois.
- **Il dénombre, il ne garantit pas**, et il le dit dans son propre rapport. Il lit les chemins d'import sans compiler, ce qui est le prix de tourner sur une cible non installée. Ce qui garantit, c'est `eslint-config-maedow-arch`, une fois la migration faite.
- **Un projet sans `features/` ni `core/` reçoit une phrase, pas un silence.** Sans elle, les règles de frontière n'auraient rien vérifié et le rapport vide se serait lu comme une bonne nouvelle. C'est le premier défaut rencontré sur un projet réel, qui rangeait tout dans `app/`.
